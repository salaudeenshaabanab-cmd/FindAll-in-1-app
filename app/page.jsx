...
export default function Home() {
  ...
  const handleAddProduct = (e) => {
    ...
    const newItem = {
      ...
      physicalCondition: newCondition === 'Brand New' ? 'Flawless / Sealed' : 'Clean used condition',
      location: 'Oyo State, Ibadan',
      image: newImage || 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=600',
      description: newDescription || 'No additional details provided.',
    };
  ...
  const handleDeleteProduct = (id) => {
    if (confirm('Are you sure you want to permanently delete this product from your store?')) {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', color: '#0f172a', fontFamily: 'system-ui, sans-serif', paddingBottom: '90px', position: 'relative' }}>
              ...
              {filteredInventory.map((item) => (
                ...
                >
                  <div style={{ height: '180px', backgroundColor: '#f1f5f9', position: 'relative' }}>
                    ...
                    <span style={{ position: 'absolute', top: '10px', left: '10px', backgroundColor: item.condition === 'Brand New' ? '#16a34a' : '#ca8a04', color: '#ffffff', fontSize: '10px', padding: '3px 8px', borderRadius: '20px', fontWeight: 700 }}>
                      {item.condition}
                    </span>
                  </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', overflow: 'hidden', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ padding: '20px', backgroundColor: '#f8fafc', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '10px' }}>
                <p style={{ fontSize: '11px', color: '#64748b', textTransform: 'uppercase', fontWeight: 700, margin: '0 0 2px' }}>Condition Status</p>
            ...
            <div style={{ padding: '20px' }}>
              <h4 style={{ fontSize: '14px', fontWeight: 800, color: '#0f172a', margin: '0 0 6px' }}>Product Description</h4>
      ...
      {/* VIEW 4: FULL FEATURED ADMIN PANEL */}
      {currentView === 'admin' && (
        <section style={{ maxWidth: '700px', margin: '30px auto', padding: '0 16px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '24px', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
            ...
            {!isAdminLoggedIn ? (
              ...
              <div>
                ...
                {/* ADD PRODUCT SECTION */}
                <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0', marginBottom: '30px' }}>
                  <h3 style={{ fontSize: '15px', fontWeight: 800, marginBottom: '14px', color: '#1e3a8a' }}>➕ Publish New Product</h3>
                  <form onSubmit={handleAddProduct}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>Condition</label>
                        <select 
                          value={newCondition} 
                    ...
                    <div style={{ marginBottom: '12px' }}>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>Image URL</label>
                      <input 
                        type="text" 
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>Full Product Description</label>
                      <textarea 
                        rows="3"
                        placeholder="Describe specifications, storage, battery health, accessories included..." 
                        value={newDescription} 
                        onChange={(e) => setNewDescription(e.target.value)}
                        style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '13px', background: '#fff', resize: 'vertical' }}
                      ></textarea>
                    </div>
                ...
                <h3 style={{ fontSize: '15px', fontWeight: 800, marginBottom: '12px', color: '#1e3a8a' }}>📦 Manage Live Inventory ({inventory.length} Items)</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '350px', overflowY: 'auto' }}>
                  {inventory.map((item) => (
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f1f5f9', padding: '10px 12px', borderRadius: '8px', gap: '10px' }}>
                      <img src={item.image} alt={item.title} style={{ width: '40px', height: '40px', objectFit: 'cover', borderRadius: '6px', flexShrink: 0 }} />
                      <div style={{ flexGrow: 1, minWidth: 0 }}>
                        <p style={{ fontSize: '12px', fontWeight: 700, margin: '0 0 2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.title}</p>
                        <p style={{ fontSize: '11px', color: '#64748b', margin: 0 }}>₦ {item.price.toLocaleString()} • <span style={{ color: item.condition === 'Brand New' ? '#16a34a' : '#ca8a04', fontWeight: 700 }}>{item.condition}</span></p>
                      </div>
                      <button 
                        onClick={() => handleDeleteProduct(item.id)}
                        style={{ backgroundColor: '#ef4444', color: '#ffffff', border: 'none', padding: '6px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}
                      >
                        Delete 🗑️
                      </button>
                    </div>
